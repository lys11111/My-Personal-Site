...
| 类型 | 做法 |
|------|------|
| 列表/卡片封面 | frontmatter `cover` → `public/images/projects/` |
| 详情页 Demo 视频 | frontmatter `coverVideo` → `public/videos/projects/`（`poster` 用 `cover`） |
| 正文配图 | `public/images/` → `![说明](/images/xxx.png)` |
| 正文内嵌视频 | `<video src="/videos/..." controls>` 或 `coverVideo` |
| 可下载文件 | `public/files/` → 正文链接 |
| Demo / GitHub | frontmatter `demoUrl`、`repoUrl`（详情页顶部按钮） |
| 内嵌网页 | 正文 `<iframe>`（目标站须允许嵌入） |

**注意**：单文件建议 < 20MB，Cloudflare 部署上限约 25MB。Markdown 表格在详情页已全宽均分排版（`global.css` → `.prose-content table`）。

更完整说明与 **7 个项目总览** 见 [`PROJECT_CONTEXT.md`](../PROJECT_CONTEXT.md#项目总览2026-05-31)。
